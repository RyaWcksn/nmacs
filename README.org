...
#+SUBTITLE: Config for GO, Java and Flutter development, also note taking!
...
#+RESULTS:
| 有 | %e | mode-line-front-space | mode-line-frame-identification | mode-line-buffer-identification |   | mode-line-position | evil-mode-line-tag | (:eval (if vc-mode (let* ((noback (replace-regexp-in-string (format ^ %s (vc-backend buffer-file-name))   vc-mode)) (face (cond ((string-match ^ - noback) 'mode-line-vc) ((string-match ^ [:@] noback) 'mode-line-vc-edit) ((string-match ^ [!\?] noback) 'mode-line-vc-modified)))) (format  %s (substring noback 2))))) |   | (:eval (when (and (bound-and-true-p lsp-mode) (lsp--client-servers)) (format LSP[%s] (mapconcat 'lsp--client-server-id (lsp--client-servers) , )))) | mode-line-misc-info | mode-line-end-spaces |
...
#+RESULTS:
| tree-sitter-hl-mode | flycheck-golangci-lint-setup | (lambda nil (flycheck-golangci-lint-setup) (setq flycheck-local-checkers '((lsp (next-checkers golangci-lint))))) | yas-minor-mode | lsp-go-install-save-hooks | company-mode | flycheck-mode | lsp-deferred |
...
#+RESULTS:
| lsp-deffered | tree-sitter-hl-mode | lsp-java-boot-lens-mode | lsp-deferred | flycheck-mode | company-mode |
...
#+RESULTS:
| tree-sitter-hl-mode | cargo-minor-mode | lsp-deferred | company-mode |
